...
export default function Register() {
  ...
  const handleGoogle = async () => {
    ...
    if (error) setError(t('register.error_generic'))
  }
        ...
        <div className="bg-surface border border-edge rounded-2xl p-8">
          {submitted ? (
            <div className="text-center py-4">
              <h1 className="font-syne text-2xl text-text-main mb-2">{t('register.success_title')}</h1>
              <p className="text-sm text-muted">{t('register.success_subtitle')}</p>
              <Link to="/login" className="inline-block mt-6 text-sm text-gold hover:underline">
                {t('auth.login_link')}
              </Link>
            </div>
          ) : (
            <>
              <h1 className="font-syne text-2xl text-text-main mb-1">{t('register.title')}</h1>
              <p className="text-sm text-muted mb-8">{t('register.subtitle')}</p>

              <form onSubmit={handleSubmit} className="space-y-5">
                {/* Full name */}
                <div>
                  <label htmlFor="fullName" className="block text-xs font-jetbrains text-muted mb-2 uppercase tracking-wider">
