...
- **CRM** (`/crm`): reemplaza el `href: null` del nav — página promocional estática (sin tabla en base de datos ni admin, mismo patrón que Curso de Rentas) para el CRM externo "GoHighLevel": beneficios del entrenamiento, precio preferencial ($77/mes vs. $97/mes regular), enlace al recorrido de la plataforma y CTA para abrir la cuenta.
- **Oficinas** (`/oficinas`): reemplaza el `href: null` del nav — catálogo estático (sin tabla en base de datos ni admin, decisión explícita) de las 4 oficinas físicas del equipo en Florida (Miami, Orlando, Tampa, West Palm Beach): dirección, horario, mapa de Google Maps embebido por oficina (iframe sin API key) y botón de reserva de sala de juntas por Calendly, uno distinto por oficina.
...
5. **Ítems de navegación sin ruta** (`components/estudiante/nav-config.ts`, `href: null`): Acelerador Pro/Starter, Transacciones.
