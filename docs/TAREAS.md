...
4. **Panel de admin** (`/admin`) — igual, stub sin UI. Falta gestión de usuarios/roles, cursos, equipo/galería (hoy esas dos últimas solo se editan a mano por SQL).
5. **Secciones "en construcción" del área de estudiante**: `/calendario`, `/herramientas`, `/marketing`, `/soporte`. Ver alcance objetivo en `docs/descripcion_contenido_pagina.txt` (sitio de referencia).
