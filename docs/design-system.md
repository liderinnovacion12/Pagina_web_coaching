...
| `whatsapp` / `whatsapp-dark` | `#25D366` / `#1EBE5D` | **Uso acotado** — botón "Únete a los Grupos y Comunidades de WhatsApp" en `/dashboard`, y acento de canal (icono/borde/link) para grupos de WhatsApp en `GrupoCard` (`/herramientas`). Reservado para representar el canal/marca WhatsApp específicamente — no usar como acento decorativo genérico. |
