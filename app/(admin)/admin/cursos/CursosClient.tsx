...
function NuevoCursoForm() {
  ...
  const f = tr.admin.cursos.form;
  const c = tr.admin.cursos;
  ...
  return (
    <div>
      {!open ? (
        ...
        >
          <Plus className="h-4 w-4" /> {c.nuevoCurso}
        </button>
      ) : (
        <div className="rounded-2xl border border-gold-500/25 bg-ink-900/80 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display text-base font-semibold text-white">{c.nuevoCurso}</h3>
            <button onClick={() => setOpen(false)} className="text-mist-500 hover:text-white"><X className="h-4 w-4" /></button>
          </div>
          <form action={action} className="space-y-4">
            <div>
              <label className="block font-mono text-xs text-mist-400 mb-1.5 uppercase tracking-wider">{f.titulo} *</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block font-mono text-xs text-mist-400 mb-1.5 uppercase tracking-wider">{f.precio}</label>
              ...
              <div>
                <label className="block font-mono text-xs text-mist-400 mb-1.5 uppercase tracking-wider">{f.categoria}</label>
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                ...
                {pending ? f.guardando : f.crearCurso}
              </button>
  ...
  return (
    <form action={formAction} className="space-y-4 pt-4">
      ...
      <div className="flex gap-3">
        ...
        >
          {pending ? f.guardando : f.guardarCambios}
        </button>
        <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-mist-400 hover:text-white">{f.cancelar}</button>
      </div>
    </form>
  );
}
    ...
    <div className="rounded-2xl border border-white/8 bg-ink-900/60 p-5 transition-all hover:border-white/12">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className={`rounded-full border px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider ${
              ...
              {curso.publicado ? c.publicado : c.borrador}
            </span>
          ...
          <p className="mt-1 font-mono text-xs text-mist-500">
            ${curso.precio.toFixed(2)} · {curso.totalLecciones} {c.lecciones}
          </p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {c.leccionesBtn} <ChevronRight className="h-3 w-3" />
          </Link>

          <form action={async () => { await togglePublicadoAction(curso.id, !curso.publicado); }}>
            <button type="submit" title={curso.publicado ? c.despublicar : c.publicar}
...
export function CursosClient({ cursos }: { cursos: CursoAdmin[] }) {
  const { tr } = useLanguage();
  const c = tr.admin.cursos;
  const publicados = cursos.filter((x) => x.publicado).length;
  const borradores = cursos.filter((x) => !x.publicado).length;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold text-white">{c.titulo}</h1>
          <p className="mt-1 font-mono text-xs text-mist-400">
            {publicados} {c.publicados} · {borradores} {c.borradores}
          </p>
        </div>
        <NuevoCursoForm />
      </div>
      ...
      {cursos.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/10 py-16 text-center">
          ...
          <p className="text-mist-400">{c.sinCursos}</p>
        </div>
      ) : (
