...
function LeccionRow({
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-ink-900/60 px-4 py-3 transition-all hover:border-white/12">
      <div className="flex items-center gap-4">
        ...
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-ink-800 border border-white/8">
          {({ video: <Video className="h-4 w-4 text-blue-400" />, pdf: <FileText className="h-4 w-4 text-gold-400" /> } as Record<string, React.ReactNode>)[leccion.tipo_contenido] ?? <Play className="h-4 w-4 text-mist-400" />}
        </div>
...
export function LeccionesClient({
  lecciones, cursoId, cursoTitulo, estadisticas,
}: {
